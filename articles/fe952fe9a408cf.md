...
今回は、先日 [Okta を Terraform と GitHub Actions を使ってコード管理するチュートリアル](https://zenn.dev/shimosyan/books/bb0ba712133779061804) で書いた Okta の管理を Terraform を使って管理しやすくする方法の補足です。
「こんな使い方もできるよ！」といった Tips を紹介できたらと思います。

## Okta でミスると怖いところ、Terraform で解決したいところ

Okta と SaaS を連携しようとすると、Okta 側では Application で対象の SaaS を誰がどう使えるかに関する設定を実施すると思います。

このとき、Okta のユーザーと Application を紐付けるときは、Application 設定内の `Assignments` にユーザーもしくはグループを紐付けると思います。
...
- 並び替えに画面のスクロールを要して操作しづらい
- 並び位置が違う
- うっかり操作ミスで並び替えを変えてしまう
- 並び替えのたび、 SaaS にプロビジョニング処理されて意図しない同期がされる

これらのように手作業では大変辛くなってきたので、これを解決したいと思います。
...
このあと記述する `okta_app_group_assignments` では、割り当てる Applications の `app_id` を指定する必要があります。しかし、この Applications は Terraform から作成していないため Okta から ID を取得する必要があります。

それには、`data` 句を使います。
Okta 上の Applications の一覧に対して `data` 句で指定した `label` と同じ Application 名を検索します。`data` 句は検索にヒットした Applications のプロパティを Terraform で使えるようにします。
...
特に、Organization Units が大量にある Google Workspace や、ライセンスやロールが複数組み合わせとなる Microsoft 365 の Provisioning に大きく貢献できるかと思います！
