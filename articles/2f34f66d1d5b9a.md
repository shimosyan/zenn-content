...
つまるところ、本問題を回避するには MSI 形式のインストーラでも「Windows アプリ（Win32）」で作成する必要があるわけです。結局、「基幹業務アプリ」だったら MSI をサクッと差し替えができるところが、課題1が重たい「Windows アプリ（Win32）」になってしまうわけです…。
...
インストール処理を選ぶためにアプリケーション本体が保存されるパスを調べておきます。パスが `C:¥Users¥` 配下かそうでないかを調べておくだけでOKです。
