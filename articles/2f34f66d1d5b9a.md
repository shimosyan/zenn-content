...
if ($uninstall)
{
  ...
  if ($productId32bit -eq "" -and $productId64bit -eq ""){
    Write-Host "Application not found at registry."
    exit 1
  }

  $productId = ""
  if (!($productId32bit -ne "" -or $productId64bit -ne "")){
    Write-Host "Exist 32bit and 64bit applications. I use 64bit version."
...
#
if ($installerUrl -eq "") {
  Write-Error "Must input Application Installer URL"
}
...
# Execution install on silent mode
Start-Process -NoNewWindow -PassThru -Wait -FilePath "msiexec" -ArgumentList "/i $installerName $appendArguments /qn"
...
```sh
powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Label' 'URL' 'Argument'"
```
...
|`Argument`|MSI ファイルに渡す追加の引数。特にない場合は指定しなくてOK|
...
|1Password|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 '1Password' 'https://downloads.1password.com/win/1PasswordSetup-latest.msi' '/norestart MANAGED_INSTALL=1'"`|
...
|1Password|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 '1Password' -uninstall"`|
