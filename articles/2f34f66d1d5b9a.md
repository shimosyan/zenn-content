...
![プログラム画面の入力例](/images/intune_msi_installer/intune_program.png =360x)
...
```sh
powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Label' 'Path' 'URL' MaxWaitTime"
```

|コマンド内の文字列|置き換える値|
|---|---|
|`Label`|事前準備で確認した「アプリケーションのタイトル」|
|`Path`|事前準備で確認した「アプリケーション本体のファイルパス」|
|`URL`|事前準備で確認した「MSI インストーラーの公開 URL」|
|`MaxWaitTime`|インストール処理を待機する最大の待ち時間。デフォルトは`120`（省略可）|

例えば、例に挙げた Google Chrome や Zoom ではこんな感じです。

|アプリケーション|コマンド例|
|---|---|
|Google Chrome (64bit)|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Google Chrome' 'C:\Program Files\Google\Chrome\Application\chrome.exe' 'https://dl.google.com/dl/chrome/install/googlechromestandaloneenterprise64.msi' 180"`|
|Zoom (64bit)|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Zoom Workplace (64-bit)' 'C:\Program Files\Zoom\bin\zoom.exe' 'https://zoom.us/client/latest/ZoomInstallerFull.msi?archType=x64'"`|

:::message
Google Chrome はインストール処理に時間がかかることがあるので、`MaxWaitTime` に `180` を指定しています。
:::
...
```sh
powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Label' 'Path' -uninstall"
```
...
|アプリケーション|コマンド例|
|---|---|
|Google Chrome (64bit)|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Google Chrome' 'C:\Program Files\Google\Chrome\Application\chrome.exe' -uninstall"`|
|Zoom (64bit)|`powershell -ExecutionPolicy Bypass ".\msiInstaller.ps1 'Zoom Workplace (64-bit)' 'C:\Program Files\Zoom\bin\zoom.exe' -uninstall"`|
