...
その 1 では Okta に新規で設定を追加するという基本的な操作をしました。
...
既に Okta のグループを作り込んでいる場合、一から組み直すのは難しいと思いますがそれを解決します。
...
ここではグループルールの作成とグループへの割り当てを指定します。

Web コンソール上でグループルールを編集する場合、割り当て先のグループはすぐに変更できません。また、条件式を編集するときでも一度 Inactive にする必要があるなど少し使いづらい点があります。

しかし、Terraform ではこれらをすべて解決してくれます。
（内部的には再作成や Inactive・Active を高速で処理してるだけですが）

今回はシンプルにグループとグループルールを 1 つずつ作成し、そのグループルールの割り当て先をグループに指定します。
...
このグループ ID は前述のように Okta から引っ張ってくることもできます。
もちろん、Terraform で管理下においているグループであれば、`okta_group.RESOURCE_ID.id`と記述することで、上記であれば`group_by_rule_1`のグループ ID を参照できます。
...
しかし、そのグループを作り直してしまうと ID が変わってしまいます。結果、Terraform のコードが正しく動かない事態に陥ります。
...
- グループ 2 は Terraform 管理にしない。名前は変わらないが、将来 ID は変更される可能性がある。
